...
const MonthlyExpensesList = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        ...
        const response = await axios.get(`/api/expense/monthly/${month}/${year}`, {
          headers: {
        ...
        alert('Error fetching expenses list');
      }
    };
  ...
  const downloadPDF = async () => {
    try {
      ...
      const response = await axios.get(`/api/expense/download/${month}/${year}`, 
        {
          responseType: 'blob' ,
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      link.href = url;
      link.setAttribute('download', `ExpenseList_${month}_${year}.pdf`);
  ...
  return (
    <div className="p-6 bg-white shadow-md rounded-lg">
      <h2 className="text-2xl font-bold mb-4">Expense List for {month}/{year}</h2>
        ...
        <tbody>
          {expenses.map((expense) => (
            <tr key={expense._id}>
              ...
              <td className="py-2 px-4 border-b">{`${expense.price}`}</td>
              <td className="py-2 px-4 border-b">{`${expense.amount} units`}</td>
